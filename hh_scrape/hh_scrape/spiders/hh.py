...
#         # Открываем JSON-файл и загружаем список URL-ов
#         with open('vacancies.json', 'r') as file:
#             pages = json.load(file)

#         # Проходим по каждому URL и делаем запрос с помощью Scrapy
#         for page in pages:
#             url = page['page_url']
#             yield scrapy.Request(url=url, callback=self.parse_page)

#     def parse_page(self, response):
#         # Обрабатываем ответ от страницы
#         self.logger.info(f'Обрабатываем страницу: {response.url}')
#         # Здесь можно извлекать данные и выполнять другие действия
#         # Например, извлечь заголовок страницы:
#         TITLE = '//a[@class="magritte-link___b4rEM_4-2-10"]/text()'
#         HREF = '//a[@class="magritte-link___b4rEM_4-2-10"]/@href'
#         SOLARY = '//span[@class="magritte-text___pbpft_3-0-14"]/text()'
#         EXPERTISE = '//div[@class="magritte-tag___WdGxk_3-0-11"]/text()'

#         title = response.xpath('//span[@data-qa="serp-item__title-text"]/text()').get()
#         title1 = response.xpath('//a[@data-qa="vacancy-serp__vacancy-title"]/text()').get()

#         # href = response.xpath("//a[@data-qa='serp-item_title']/@href").get()
#         # solary = response.xpath(SOLARY).get()
#         # expertise = response.xpath(EXPERTISE).get()

#         self.logger.info(f'Заголовок страницы: {title}')     
#         self.logger.info(f'Ссылка страницы: {title1}')     
